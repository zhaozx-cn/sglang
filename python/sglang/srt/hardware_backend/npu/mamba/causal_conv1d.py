...
def causal_conv1d_fn_native(
    ...
    if initial_states is None:
        ...
        #print(f"*********************rank {torch.distributed.get_rank()} x ",x.reshape(-1,x.shape[-1])[:10,:10]," shape ",x.shape,flush=True)
    ...
    #print(f"*********************rank {torch.distributed.get_rank()} out ",out.reshape(-1,out.shape[-1])[:10,:10]," shape ",out.shape,flush=True)
...
def causal_conv1d_fn_npu(
    ...
    **kwargs,
    ...
    #    print(f"*****************rank {torch.distributed.get_rank()} x shape ",x.shape," conv_states shape ",conv_states.shape," cache_indices ",cache_indices,flush=True)
    ...
    #if torch.distributed.get_rank() == 2:
    #    print(f"*****************rank {torch.distributed.get_rank()} x_pad shape ",x_pad.shape," initial_state_pad ",initial_state_pad," seqlens ",seqlens," indices ",indices," conv_states ",conv_states.shape,flush=True)
    ...
    out, final_states_out = causal_conv1d_fn_native(
        ...
        initial_states=conv_states[cache_indices[0]][..., : (width - 1)],
    ...
    #print(f"*********************rank {torch.distributed.get_rank()} out_final ",out_final.reshape(-1,out_final.shape[-1])[:10,:10]," shape ",out_final.shape," pad_seq_len ",pad_seq_len," width ",width,flush=True)
