...
# Reproducible four-node Kimi-K3 DSpark graph launcher for cache-off prefill.
#
...
# DP=4 normalizes the global chunk by four. A global 65536 keeps one 8K input
# inside a single per-DP 16K cache-off chunk.
...
echo "Kimi-K3 DSpark rank=${NODE_RANK}/4 TP=${TP_SIZE} DP=${DP_SIZE} chunk=${CHUNKED_PREFILL_SIZE} cache=off block=${DSPARK_BLOCK_SIZE} graph=[${GRAPH_BS[*]}]"
...
python3 -m sglang.launch_server "${SERVER_ARGS[@]}" 2>&1 | \
    tee "${LOG_DIR:-${REPO_ROOT}/logs}/k3_dspark_cacheoff_rank${NODE_RANK}_$(date +%Y-%m-%d_%H-%M-%S).log"
