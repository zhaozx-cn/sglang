...
# ---------------------------------------------------------------------------
# SiTU activation (Kimi-K3 NPU path).
#
# Requires the triton-ascend (CANN) fork and sgl_kernel_npu. The imports are
# guarded so this module stays importable on non-NPU hosts; situ_and_mul raises
# at call time if the NPU stack is missing.
# ---------------------------------------------------------------------------
try:
    import triton
    ...
    _SITU_AVAILABLE = True
except Exception:
    _SITU_AVAILABLE = False
    ...
    def _situ_and_mul_kernel(
        ...
        if HAS_GROUP_LIST:
            if GROUP_LIST_TYPE == 0:  # cusum
                ...
                group_list = tl.load(
                    group_list_ptr + gl_offsets, gl_mask, other=0
                ).to(tl.int32)
        ...
        # rows distributed across vector cores (manual split + early return).
        ...
        # H-tile over the OUTPUT dim (HALF_COLS): out[i] only needs gate[i]=x[i]
        # and up[i]=x[d+i], so every [h:h+BLOCK_H] tile is self-contained.
        h_offs = tl.arange(0, BLOCK_H)
        for row_idx in range(row_begin, row_end):
            # int64 row offset: row_idx * stride stays int32 by default on
            # triton-ascend (no auto-promote), which overflows when N*d is large.
            ...
            for h_start in range(0, HALF_COLS, BLOCK_H):
                ...
                gate = tl.load(
                    x_ptr + row_off + h_idx, mask=mask, other=0.0
                ).to(tl.float32)
                up = tl.load(
    ...
    def situ_and_mul(
        ...
        Args:
            ...
            group_list: per-expert token counts (count) or cumulative sum (cusum).
                ``None`` = dense / shared-expert path: process ALL rows of ``x``.
                Required only for the routed-MoE path.
        ...
        Returns:
            ``[..., d]`` tensor. With ``group_list``: only the first
            ``sum(group_list)`` rows are written (rest is padding). Without: all
            rows written.
        """
        ...
        if has_group_list:
            ...
            # dense / shared: kernel skips the group_list block, so these are
            # never read -- pass harmless dummies.
            group_list_arg = x_2d
            num_experts_arg = 1
...
else:  # _SITU_AVAILABLE is False

    def situ_and_mul(*args, **kwargs):  # type: ignore[no-redef]
        raise RuntimeError(
            "situ_and_mul requires triton-ascend (CANN) and sgl_kernel_npu; "
