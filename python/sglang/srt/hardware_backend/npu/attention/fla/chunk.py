...
from sglang.srt.hardware_backend.npu.attention.fla.chunk_delta_h import (
    chunk_gated_delta_rule_fwd_h_npu as chunk_gated_delta_rule_fwd_h,
)
from sglang.srt.hardware_backend.npu.attention.fla.chunk_o import chunk_fwd_o_npu as chunk_fwd_o
from sglang.srt.hardware_backend.npu.attention.fla.chunk_scaled_dot_kkt import (
    chunk_scaled_dot_kkt_fwd_npu as chunk_scaled_dot_kkt_fwd,
)
from sglang.srt.hardware_backend.npu.attention.fla.cumsum import chunk_local_cumsum
from sglang.srt.hardware_backend.npu.attention.fla.l2norm import l2norm_fwd
from sglang.srt.hardware_backend.npu.attention.fla.solve_tril import solve_tril_npu as solve_tril
...
from sglang.srt.hardware_backend.npu.attention.fla.wy_fast import recompute_w_u_fwd_npu as recompute_w_u_fwd
...
def chunk_gated_delta_rule_fwd(
    ...
    #    print(f"***************rank {torch.distributed.get_rank()} h ",h.reshape(-1,128)[-10:,:10]," shape ",h.shape,flush=True)
    #    print(f"***************rank {torch.distributed.get_rank()} final_state ",final_state.reshape(-1,128)[-10:,:10]," shape ",final_state.shape,flush=True)
    if get_attention_cp_group().world_size > 1:
        ...
        #if torch.distributed.get_rank() % 2 == 0:
        #    print(f"***************rank {torch.distributed.get_rank()} h_update ",h_update.reshape(-1,128)[-10:,:10]," shape ",h_update.shape,flush=True)
