...
# Reproducible four-node Kimi-K3 DSpark graph launcher.
#
...
echo "Kimi-K3 DSpark rank=${NODE_RANK}/4 TP=${TP_SIZE} DP=${DP_SIZE} block=${DSPARK_BLOCK_SIZE} graph=[${GRAPH_BS[*]}]"
...
python3 -m sglang.launch_server "${SERVER_ARGS[@]}" 2>&1 | \
    tee "${LOG_DIR:-${REPO_ROOT}/logs}/k3_dspark_rank${NODE_RANK}_$(date +%Y-%m-%d_%H-%M-%S).log"
